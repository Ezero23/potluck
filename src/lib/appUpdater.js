...
// Collect PIDs of all potluck-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from potluck install dir or wrapper cli.js
        const isAppProcess = lower.includes("potluck") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("potluck") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "potluck");
  }
  return path.join(os.homedir(), ".potluck");
}
...
// Resolve npx/potluck binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx potluck` — works regardless of global bin path changes after npm i -g
