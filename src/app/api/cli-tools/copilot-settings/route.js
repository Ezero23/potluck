...
const hasPotluckConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Potluck");
};

const getPotluckEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Potluck") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getPotluckEntry(config);
...
// POST - Apply Potluck config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Potluck entry or append
    const idx = config.findIndex((e) => e.name === "Potluck");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Potluck entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Potluck");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Potluck removed from Copilot config",
    });
  } catch (error) {
