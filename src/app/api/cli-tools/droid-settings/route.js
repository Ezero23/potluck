...
// Check if settings has Potluck customModels
const hasPotluckConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Potluck"));
};
...
// POST - Update Potluck customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Potluck configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Potluck"));
...
// DELETE - Remove Potluck customModels only (keep other settings)
