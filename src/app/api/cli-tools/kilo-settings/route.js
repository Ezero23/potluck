...
const hasPotluckConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["potluck"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("potluck");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasPotluck: hasPotluckConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "Potluck", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Potluck settings removed from Kilo Code" });
  } catch (error) {
