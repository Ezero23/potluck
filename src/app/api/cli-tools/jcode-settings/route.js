...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-potluck.env");
};
...
export async function POST(request) {
  try {
    ...
    env.JCODE_POTLUCK_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile potluck",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["potluck"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_POTLUCK_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "potluck configuration removed from jcode",
    });
  } catch (error) {
