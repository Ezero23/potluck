...
// Check if settings has Potluck config
const hasPotluckConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["potluck"];
};

// Read per-agent models.json and return current model id (without "potluck/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["potluck"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasPotluck: hasPotluckConfig(settings),
...
// POST - Update Potluck settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `potluck/${model}`;

    // Remove all old potluck/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("potluck/"))
    ...
    // Add fresh potluck models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`potluck/${m}`] = {};
    });

    // Remove old potluck model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("potluck/")) {
    ...
    // Update models.providers.potluck with all models
    settings.models.providers["potluck"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `potluck/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove Potluck settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Potluck from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["potluck"];
    ...
    // Remove potluck models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("potluck/"));
    ...
    // Reset agents.defaults.model.primary if it uses potluck
    if (settings.agents?.defaults?.model?.primary?.startsWith("potluck/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "Potluck settings removed successfully",
    });
  } catch (error) {
