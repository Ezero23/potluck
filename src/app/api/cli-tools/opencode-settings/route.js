...
const hasPotluckConfig = (config) => {
  ...
  return !!config.provider["potluck"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["potluck"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasPotluck: hasPotluckConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("potluck/") ? config.model.replace(/^potluck\//, "") : null,
...
// POST - Apply Potluck as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_potluck";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing potluck provider entry and its models
    const existingProvider = config.provider["potluck"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["potluck"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `potluck/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `potluck/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("potluck/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Potluck provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["potluck"]?.models) {
      delete config.provider["potluck"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["potluck"].models).length === 0) {
        delete config.provider["potluck"];
        if (config.model?.startsWith("potluck/")) delete config.model;
      } else if (config.model === `potluck/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["potluck"].models);
        config.model = `potluck/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire potluck provider
      if (config.provider) delete config.provider["potluck"];
      if (config.model?.startsWith("potluck/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("potluck/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Potluck settings removed from OpenCode",
    });
  } catch (error) {
