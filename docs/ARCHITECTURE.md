...
Potluck is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[Potluck Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.potluck/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.potluck/usage.json`, `~/.potluck/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.potluck/db.json`)
- usage stats: `~/.potluck/usage.json`
- request log lines: `~/.potluck/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Potluck Runtime]
        Next[Next.js Server\nPORT=21023]
...
1. `usageDb` currently stores under `~/.potluck` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/potluck && npm run build`
- Build Docker image: `cd /root/dev/potluck && docker build -t potluck .`
- Start service and verify:
- `GET /api/settings`
- `GET /api/v1/models`
- CLI target base URL should be `http://<host>:21023/v1` when `PORT=21023`
