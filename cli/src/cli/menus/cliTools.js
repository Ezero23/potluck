...
async function buildCodexHeader() {
  ...
  const { installed, hasPotluck, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasPotluck, settings } = result.data;
  ...
  // Extract Potluck custom model config
  const custom = settings?.customModels?.find(m => m.id?.startsWith("custom:Potluck"));
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract Potluck provider config
  const provider = settings?.models?.providers?.["potluck"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("potluck/") ? primary.replace("potluck/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasPotluck, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use Potluck\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
